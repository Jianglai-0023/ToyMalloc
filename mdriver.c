...
static void run_tests(int num_tracefiles, char trace_from_stdin,
                      ...
                      stats_t *mm_stats, range_t *ranges,
...
int main(int argc, char **argv) {
    ...
    run_tests(num_tracefiles, trace_from_stdin, tracedir, tracefiles, mm_stats,
              ranges, &speed_params);
...
static double eval_mm_util(trace_t *trace, int tracenum) {
    ...
    for (i = 0; i < trace->num_ops; i++) {
        switch (trace->ops[i].type) {
            ...
            default:
                app_error("trace %d: Nonexistent request type in eval_mm_util", tracenum);
        }
...
static void printresults(int n, stats_t *stats) {
    ...
    for (i = 0; i < n; i++) {
        if (stats[i].valid) {
            printf("%2s%4s %5.0f%%%8.0f%10.6f%9.0f %s\n",
                   stats[i].weight != 0 ? "*" : "", "yes", stats[i].util * 100.0,
    ...
    if (errors == 0) {
        ...
        printf("%2d     %5.0f%%%8.0f%10.6f%9.0f\n", sumweight,
               (sumutil / (double)sumweight) * 100.0, sumops, sumsecs,
